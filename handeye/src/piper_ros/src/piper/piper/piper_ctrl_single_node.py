...
class PiperRosNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.publisher_thread = threading.Thread(
            target=self.publish_thread, daemon=True)
...
def main(args=None):
    ...
    try:
        ...
        disconnect_thread = threading.Thread(
            target=piper_single_node.piper.DisconnectPort,
            kwargs={'thread_timeout': 0.2}, daemon=True)
        disconnect_thread.start()
        disconnect_thread.join(timeout=1.0)
        if disconnect_thread.is_alive():
            piper_single_node.get_logger().warn(
                'Piper SDK CAN cleanup timed out; the process will release the socket')
